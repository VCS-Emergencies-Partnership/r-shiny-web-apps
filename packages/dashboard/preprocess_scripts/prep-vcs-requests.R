...
requests <- requests %>% filter(!is.na(request_date)) %>%
  ...
  mutate('TacticalCell'=case_when(multi_agency_cell == 'South West and the Channel Islands' ~ 'South and the Channel Islands',
...
requests_geocoded <- left_join(requests, postcode2lad, by='postcode')

all_requests <- requests_geocoded %>% mutate('request_status'=case_when(!grepl("Closed", status) ~ 'Active',
