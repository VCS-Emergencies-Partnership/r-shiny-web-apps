...
# create color scale that encodes two variables --> original colour scale. 
...
bivariate_color_scale <- tibble(
  ...
  "2 - 3" = "#b36600",
  "1 - 3" = "#f3b300", # low inequality, high income #f0f0f0
  "3 - 2" = "#376387",
...
g.legend<-
  ...
  #theme(axis.title=element_text(color="black", size=12), axis.title.y.right= element_text(angle=90, vjust=8, hjust=0.5), axis.title.x.top=element_text(vjust=-8, hjust=0.4)) +
  theme(axis.title=element_text(color="black", size=40), axis.title.y= element_text(angle=90, vjust=-8, hjust=0.5)) +
  coord_fixed() +
  annotate("segment", x=0.5, y=3.75, xend=3.5, yend=3.75, size=3,
           col="black", arrow=arrow(length=unit(0.3, "cm"))) +
  annotate("segment", x=0.2, y=3.5, xend=0.2, yend=0.5, size=3,
           col="black", arrow=arrow(length=unit(0.3, "cm"))) + 
  annotate(geom="text", x=3, y=3, label="Most in\n need", color="white", size=10.5438) + #4.233333333
  annotate(geom="text", x=1, y=1, label="Least in\n need", color="black", size=10.5438)
...
par()
g.legend
dev.print(png,'/home/izzy-everall/r-shiny-web-apps/packages/dashboard/www/bivar-legend_v2.png', bg="transparent",width=800, height=600)
dev.off()
